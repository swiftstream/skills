...
When accepted central `main` advances while an initial same-repository manual request is still based on an older accepted main, the trusted `Federation main advance` workflow may normalize that request entirely inside GitHub. This automatic normalization is allowed only before an immutable RequestAnchor exists and only after trusted code proves that the current request head differs from its own recorded historical PR base by exactly the valid initial `.federation-request` marker shape for ADD, UPDATE, or REMOVE. `Federation main advance` and every `Federation interactive` run share the same repository-wide GitHub Actions concurrency group with cancellation disabled and `queue: max`, so concurrent authority-bearing runs are serialized without silently replacing an older pending run. Source topology also enforces creation capability: only the explicit interactive anchor path may call `create_anchor_once`; proposal regeneration, state finalization, and trusted validation consume an already-existing RequestAnchor and fail closed if that authority disappears. RequestAnchor creation and stale update-branch normalization therefore cannot execute concurrently: whichever trusted workflow enters the shared authority section first must finish before the other can observe or mutate that authority. Fork heads, malformed markers, anchored stale proposals, unrelated diffs, unexpected modes, or any request whose exact authority cannot be re-established remain fail-closed and are not updated automatically. A PR head with no `.federation-request` and no RequestAnchor is classified as unrelated and is a clean federation NOOP rather than a failed federation request.
